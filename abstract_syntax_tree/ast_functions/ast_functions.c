...
static ast_node_t* exectute_node(ast_node_t* node);
...
void init_ast_functions() {
    ...
    ast_functions[TAG_TK]         = ast_tag_value_provider();
    ast_functions[INT_VALUE]      = ast_int_value_provider();
    ast_functions[STRING_VALUE]   = ast_string_value_provider();
    ast_functions[ID]             = ast_reference_value_provider();
}
...
static ast_node_t* exectute_node(ast_node_t* node){
    return ast_functions[node->type].process();
}
